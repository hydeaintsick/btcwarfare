...
router.post("/login", async (req, res) => {
    try {
        ...
        const isValid = (0, adminAuth_1.verifyAdminCredentials)(email, password);
        console.log("verifyAdminCredentials result:", isValid);
        if (!isValid) {
            res.status(401).json({ error: "Invalid credentials" });
            return;
        }
        const token = (0, adminAuth_1.createAdminToken)(email);
        res.json({
            token,
            email,
...
router.get("/deposits-chart", async (req, res) => {
    try {
        ...
        const chartData = await adminService_1.default.getDepositsChartData(period);
        res.json(chartData);
    }
    catch (error) {
...
router.get("/withdrawals", async (req, res) => {
    try {
        const withdrawals = await Transaction_1.default.find({
            type: "withdrawal",
...
router.post("/withdrawals/:id/approve", async (req, res) => {
    try {
        const { id } = req.params;
        const withdrawal = await Transaction_1.default.findById(id).populate("userId", "walletAddress");
        if (!withdrawal) {
        ...
        if (!destinationAddress || !ethers_1.ethers.isAddress(destinationAddress)) {
        ...
        try {
            ...
            blockchainService_1.default.getDepositAddressSync(); // Vérifie la configuration
            ...
            const platformWallet = new ethers_1.ethers.Wallet(privateKey, provider);
            ...
            const tx = await platformWallet.sendTransaction({
                ...
                value: ethers_1.ethers.parseEther(userAmount.toString()),
            });
