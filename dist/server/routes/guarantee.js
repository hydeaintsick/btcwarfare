 ...
 */
router.get("/guaranteetx/:key", async (req, res) => {
    try {
        // VALIDATION: Vérifier la clé API
        const providedKey = req.params.key;
        const privateApiKey = process.env.PRIVATE_API_KEY;
        if (!privateApiKey) {
            console.error("[GUARANTEE] PRIVATE_API_KEY not configured in environment variables");
            res.status(500).json({
                success: false,
        ...
        if (providedKey !== privateApiKey) {
            console.warn(`[GUARANTEE] Unauthorized access attempt with key: ${providedKey.substring(0, 10)}...`);
            res.status(401).json({
                success: false,
        ...
        console.log("[GUARANTEE] Starting transaction guarantee scan for last 24 hours...");
        ...
        console.log(`[GUARANTEE] Scanning blocks ${fromBlock} to ${currentBlock} (last 24 hours)`);
        const report = {
        ...
        try {
            ...
            const actualFromBlock = Math.max(fromBlock, currentBlock - maxBlocksToScan);
            for (let blockNum = currentBlock; blockNum >= actualFromBlock; blockNum--) {
                try {
                    ...
                    for (const txHash of block.transactions) {
                        try {
                            ...
                            const result = await depositService_1.default.processDepositSecure(user, tx.hash, "ETH", amount);
                            if (result.success) {
                                report.depositsProcessed.eth++;
                                console.log(`✅ [GUARANTEE] Processed ETH deposit: ${tx.hash} (${amount} ETH)`);
                            }
                            else {
                                report.errors.push(`Failed to process ETH tx ${tx.hash}: ${result.message}`);
                                console.error(`❌ [GUARANTEE] Failed to process ETH deposit: ${tx.hash}`, result.message);
                            }
                        }
                        catch (error) {
                            report.errors.push(`Error processing ETH tx: ${error.message}`);
                            console.error(`[GUARANTEE] Error processing ETH transaction:`, error);
                        }
                    }
                }
                catch (error) {
                    report.errors.push(`Error scanning block ${blockNum}: ${error.message}`);
        ...
        try {
            ...
            const usdtAddress = network.chainId === 1n ? USDT_CONTRACT_ADDRESS_MAINNET : "0x..."; // Sepolia address si nécessaire
            ...
            for (const event of events) {
                try {
                    ...
                    // Traiter le dépôt (utiliser version sécurisée)
                    const result = await depositService_1.default.processDepositSecure(user, txHash, "USDT", amount);
                    if (result.success) {
                        report.depositsProcessed.usdt++;
                        console.log(`✅ [GUARANTEE] Processed USDT deposit: ${txHash} (${amount} USDT)`);
                    }
                    else {
                        report.errors.push(`Failed to process USDT tx ${txHash}: ${result.message}`);
                        console.error(`❌ [GUARANTEE] Failed to process USDT deposit: ${txHash}`, result.message);
                    }
                }
                catch (error) {
                    report.errors.push(`Error processing USDT tx: ${error.message}`);
                    console.error(`[GUARANTEE] Error processing USDT transaction ${event.transactionHash}:`, error);
                }
            }
        }
        catch (error) {
        ...
        const totalProcessed = report.depositsProcessed.eth + report.depositsProcessed.usdt;
        const totalFound = report.depositsFound.eth.length + report.depositsFound.usdt.length;
        console.log(`[GUARANTEE] Scan completed: ${totalProcessed} deposits processed, ${totalFound} found, ${report.depositsSkipped.alreadyProcessed} already processed`);
        res.json({
            success: true,
