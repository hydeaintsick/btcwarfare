...
const platformConfigService_1 = __importDefault(require("./platformConfigService"));
...
class BlockchainService {
    constructor() {
        ...
        this.addressCacheExpiry = 0;
        this.CACHE_TTL = 30000; // 30 secondes
    ...
    ensureProvider() {
        ...
        try {
            ...
            if (privateKey) {
                this.depositWallet = new ethers_1.ethers.Wallet(privateKey, this.provider);
    ...
    async getDepositAddress() {
        ...
        try {
            ...
            const dbAddress = await platformConfigService_1.default.getConfig('DEPOSIT_ADDRESS');
            if (dbAddress) {
     ...
     */
    async checkETHDeposit(txHash, userAddress) {
        try {
            this.ensureProvider();
        }
        catch (error) {
     ...
     */
    async checkUSDTDeposit(txHash, userAddress) {
        try {
            this.ensureProvider();
        }
        catch (error) {
        ...
        try {
            ...
            const contract = new ethers_1.ethers.Contract(usdtAddress, usdtAbi, this.provider);
            ...
            for (const event of transferEvents) {
                ...
                if (to === depositAddress.toLowerCase() &&
     ...
     */
    async checkDeposit(txHash, userAddress, currency) {
        if (currency === 'ETH') {
