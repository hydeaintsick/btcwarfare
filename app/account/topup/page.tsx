...
export default function TopupPage() {
  ...
  const { isConnected, isConnecting, sendTransaction, address, user } =
    useWallet();
  ...
  const [txStatus, setTxStatus] = useState<
    "idle" | "metaMaskPending" | "blockchainPending" | "confirmed" | "failed"
  >("idle");
  const [error, setError] = useState<string>("");
  const [blockConfirmationCount, setBlockConfirmationCount] =
    useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (
      txHash &&
      (txStatus === "blockchainPending" || txStatus === "metaMaskPending")
    ) {
  ...
  const checkTransactionStatus = async () => {
    ...
    try {
      ...
      if (result.status === "confirmed") {
        ...
        setTimeout(() => {
          router.push(
            `/account/topup/success?amount=${selectedPack}&txHash=${txHash}&credited=${result.amountAfterFee}`
          );
        }, 1500);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {selectedPack !== null && (
          ...
          >
                ...
                <div className="p-4 bg-yellow-500/20 border border-yellow-500/50 rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-yellow-400 font-medium">
                      Step 2/2: Transaction sent! Waiting for blockchain
                      confirmation...
                    </span>
                  </div>
            ...
            <button
              onClick={handlePay}
              disabled={
                loading || (txStatus !== "idle" && txStatus !== "failed")
              }
        ...
        <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          <p className="text-sm text-blue-300">
            💡 <strong>How it works:</strong> Select a pack, confirm the payment
            in MetaMask, and wait for blockchain confirmation. Your account will
            be automatically credited (amount - 5% platform fee) once the
